...
async def run_superpilot(user_configuration: dict):
    ...
    if not pilot_workspace:  # We don't have an pilot yet.
        ...
        pilot_settings: AgentSettings = SimpleAgent.compile_settings(
            client_logger,
            user_configuration,
        )
        ...
        pilot_workspace = SimpleAgent.provision_pilot(pilot_settings, client_logger)
        print("pilot is provisioned")

    # launch pilot interaction loop
    pilot = SimpleAgent.from_workspace(
        pilot_workspace,
        client_logger,
    )
