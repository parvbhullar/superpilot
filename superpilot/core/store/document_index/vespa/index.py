...
def _does_document_exist(
    ...
    Specifically in this case, document refers to a vespa document which is equivalent to a Unpod
...
def _build_vespa_filters(filters: IndexFilters, include_hidden: bool = False) -> str:
    ...
    def _build_in_filters(key: str, vals: list[str]) -> str:
        valid_vals = [val for val in vals if val]
        if not key or not valid_vals:
            return ""

        eq_elems = str(valid_vals).strip("[").strip("]")
        in_clause = f"{key} in ({eq_elems})"
        return f"({in_clause}) and "
    ...
    if tags:
        tag_attributes = []
        for tag in tags:
            if tag.tag_key and tag.tag_value and tag.tag_key != UNPOD_KN_TOKEN and tag.tag_key != UNPOD_HUB_ID:
                tag_attributes.append(tag.tag_key + INDEX_SEPARATOR + tag.tag_value)
            elif tag.tag_key == UNPOD_KN_TOKEN:
                filter_str += _build_in_filters(UNPOD_KN_TOKEN, tag.tag_value.split(","))
            elif tag.tag_key == UNPOD_HUB_ID:
                filter_str += _build_in_filters(UNPOD_HUB_ID, tag.tag_value.split(",")) # + " or access_control_list contains 'PUBLIC') "
