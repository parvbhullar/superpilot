...
class GenerateImage(Ability):
    ...
    def _check_preconditions(self, prompt: str, filename: str) -> AbilityAction | None:
        message = ""
        try:
            import requests
        except ImportError:
    ...
    def __call__(self, prompt: str, filename: str) -> AbilityAction:
        ...
        while retry_count < 10:
            ...
            if response.ok:
                try:
                    ...
                    return AbilityAction(success=False, message=message)
            else:
                try:
