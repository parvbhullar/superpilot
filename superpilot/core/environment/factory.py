...
class EnvConfigBuilder:
    ...
    def build_ability_registry(
        ...
        self.config["ability_registry"] = {
            "name": name,
            "description": description,
            "configuration": {"abilities": abilities, "config": config},
        }

    def build_memory(self, name="simple_memory", description="A simple memory"):
        self.config["memory"] = {
            "name": name,
            "description": description,
            "configuration": {},
    ...
    def build_openai_provider(
        ...
        self.config["openai_provider"] = {
            ...
            "budget": budget,
            "configuration": {"retries_per_request": retries_per_request},
        }
    ...
    def build_planning(
        ...
        self.config["planning"] = {
            "name": name,
            "description": description,
            "configuration": {"models": models, "prompt_strategies": prompt_strategies},
        }
...
if __name__ == "__main__":
    ...
    builder.build_ability_registry(
        name="ability_1",
        description="Ability Registry 1",
        abilities="some_abilities",
        config=None,
    )
    builder.build_memory(name="memory_1", description="Memory 1")
    builder.build_openai_provider(
        name="provider_1",
        description="OpenAI Provider 1",
        retries_per_request=10,
        resource_type="model",
        credentials={"api_key": None},
        budget={"total_budget": float("inf")},
    )
    builder.build_planning(
        name="planner_1",
        description="Planner 1",
        models="some_models",
        prompt_strategies="some_strategies",
    )
