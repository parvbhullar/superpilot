...
def generate_tags(input_text, model_name="gpt2", num_tags=3):
    # Load the pretrained model and tokenizer
    tokenizer = GPT2Tokenizer.from_pretrained(model_name)
    model = GPT2LMHeadModel.from_pretrained(model_name)

    # Prepare the input prompt
    prompt = f"Generate {num_tags} relevant tags for: {input_text}\nTags:"
    ...
    tags = generated_text.split("Tags:")[-1].strip().split(',')
    
    return [tag.strip() for tag in tags if tag.strip()][:num_tags]

input_text = "Artificial Intelligence is transforming industries by automating processes and enhancing decision-making."
