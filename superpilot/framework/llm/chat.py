...
from superpilot.framework.llm.base import Message
from superpilot.framework.llm.llm_utils import create_chat_completion
from superpilot.framework.llm.token_counter import count_message_tokens
...
def chat_with_ai(
    ...
    while True:
        try:
            ...
            from superpilot.autogpt.memory_management.summary_memory import (
                get_newly_trimmed_messages,
                update_running_summary,
            )

            # Insert Memories TODO remove pilot reference
            # if len(full_message_history) > 0:
            #     (
            #         newly_trimmed_messages,
            #         pilot.last_memory_index,
            #     ) = get_newly_trimmed_messages(
            #         full_message_history=full_message_history,
            #         current_context=current_context,
            #         last_memory_index=pilot.last_memory_index,
            #     )
            #
            #     pilot.summary_memory = update_running_summary(
            #         pilot,
            #         current_memory=pilot.summary_memory,
            #         new_events=newly_trimmed_messages,
            #     )
            #     current_context.insert(insertion_index, pilot.summary_memory)
            ...
            # TODO Rethink plugins implementation
            # plugin_count = len(cfg.plugins)
            # for i, plugin in enumerate(cfg.plugins):
            #     if not plugin.can_handle_on_planning():
            #         continue
            #     plugin_response = plugin.on_planning(
            #         pilot.config.prompt_generator, current_context
            #     )
            ...
            #  https://www.github.com/Torantulino/Superpilot"
            ...
            # TODO remove log cycle handler
            # pilot.log_cycle_handler.log_cycle(
            #     pilot.config.ai_name,
            #     pilot.created_at,
            #     pilot.cycle_count,
            #     current_context,
