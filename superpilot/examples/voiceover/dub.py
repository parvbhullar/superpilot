...
class VoiceDub:
    def __init__(
        ...
        tgtlang,
        clone_flag=False,
        clone_voice_description=None,
        clone_voice_files=None,
        clone_name=None,
    ):
    ...
    def speech_to_text(self, audio_path):
        ...
        if self.model is None:
            raise ValueError(
                "Speech-to-text model not initialized. Call initialize_speech_to_text() first."
            )
    ...
    def translate_text(self, text, src_lang, tgt_lang):
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful assistant that translate text from one language to another and only return translated text.",
                },
    ...
    def save_audio_bytes(
        audio_bytes, output_filename, format="mp3", bitrate=None, sample_width=None
    ):
        ...
        with io.BytesIO(audio_bytes) as audio_file:
            ...
            if format == "mp3":
                ...
                with wave.open(output_filename, "wb") as wav_file:
                    ...
                    sample_width = (
                        segment.sample_width if sample_width is None else sample_width
                    )
    ...
    def text_to_audio(
        self, text, lang, output_filename, format="mp3", bitrate=None, sample_width=None
    ):
    ...
    def dub(self, output_filename):
        ...
        print("TTS")
        if self.clone_flag:
            audio_file = self.text_to_audio_clone(
                translated_text,
                self.tgtlang,
                output_filename,
                clone_name=self.clone_name,
                clone_voice_files=self.clone_voice_files,
                clone_voice_description=self.clone_voice_description,
            )
        else:
            audio_file = self.text_to_audio(
                translated_text, self.tgtlang, output_filename
            )
        return audio_file
...
if __name__ == "__main__":
    ...
    clone_voice_description = (
        "An old indian male voice with a calm tone. Perfect for commentry of cricket"
    )
    ...
    dub = VoiceDub(
        ...
        clone_voice_files=clone_voice_files,
