...
class StableDiffusionPromptModel(SchemaModel):
    ...
    height: conint(multiple_of=64, ge=128) = Field(
        512, description="Height of the image in pixels. Must be in increments of 64."
    )
    width: conint(multiple_of=64, ge=128) = Field(
        512, description="Width of the image in pixels. Must be in increments of 64."
    )
    text_prompts: List[TextPrompt] = Field(
        ..., description="An array of text prompts to use for generation. Max 5"
    )
    cfg_scale: conint(ge=0, le=35) = Field(
        7, description="How strictly the diffusion process adheres to the prompt text"
    )
    clip_guidance_preset: ClipGuidancePreset = Field(
        ClipGuidancePreset.NONE, description="Which clip guidance preset to use"
    )
    sampler: Optional[Sampler] = Field(
        None, description="Which sampler to use for the diffusion process."
    )
    ...
    steps: conint(ge=10, le=150) = Field(
        50, description="Number of diffusion steps to run"
    )
    ...
    extras: Optional[dict] = Field(
        None,
        description="Extra parameters passed to the engine for experimental features.",
    )
