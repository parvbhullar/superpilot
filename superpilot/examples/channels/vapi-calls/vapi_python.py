...
def start_vapi_call(agent, name, number, mailing_address, source_name=""):
    ...
    try:
        ...
        conversation_script = {
            "initial_greeting": {
                "english": f"Hi, this is {agent} from CuriousKid. Am I speaking with {name}?",
                "hindi": f"Namaste! {agent} bol rahi hu, CuriousKid se. Kya mai {name} se baat kar sakti hu?"
            },
            "registration_confirmation": {
                "english": f"I see you registered on our website through {source_name} and expressed interest in our Robotics and AI courses. Do you recall this registration?",
                "hindi": f"Maine dekha ki aapne {source_name} ke through register kiya tha aur Robotics aur AI courses mei interest dikhaya tha. Kya aapko yaad hai?"
            },
            "availability_check": {
                "english": "Thank you! I wanted to discuss supporting your child's STEM learning journey. Do you have a few minutes to talk?",
                "hindi": "Thanks! Mai aapke bachche ke STEM learning journey ke bare mei discuss karna chahti hu. Kya aapke paas kuch minutes hai baat karne ke liye?"
            },
            "identity_recheck": {
                "english": f"Just to confirm, is this {name}?",
                "hindi": f"Bas confirm karna chahti hu, kya yeh {name} hain?"
            },
            "student_details": {
                "english": "Could you please tell me your child's name and their grade?",
                "hindi": "Kya aap mujhe apne bachche ka naam aur grade bata sakte hain?"
            },
            "technical_requirements": {
                "english": "Does your child have access to a laptop and a stable internet connection for the sessions?",
                "hindi": "Kya aapke bachche ke paas laptop aur stable internet connection hai sessions ke liye?"
            },
            "course_intro": {
                "english": "Let me tell you about our programs based on your child's grade...",
                "hindi": "Chaliye mai aapko aapke bachche ke grade ke hisaab se humare programs ke baare mei batati hu..."
            },
            "scheduling": {
                "english": "Would you like to schedule a free diagnostic session? We have slots available in the morning between 10 AM to 12 PM, and evening between 4 PM to 7 PM.",
                "hindi": """
                Kya aap ek free diagnostic session schedule karna chahenge? Humare paas morning 10 se 12 ke beech, ya phir evening 4 se 7 ke beech slots available hain.
                """
            },
            "schedule_confirmation": {
                "english": "Perfect! I'll schedule the diagnostic session for [time]. Our STEM expert will call you at this time. Please ensure your child is available with a device (laptop/tablet) for this interactive session.",
    ...
    try:
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                # (Optionally update a call summary object if needed)
                ...
                if current_status in ['completed', 'failed']:
                    # Generate call analysis using the extracted transcript
                    if len(transcript) == 0:
                        print("No transcript messages found; cannot generate analysis.")
                        break
                    analysis = generate_call_analysis({"messages": transcript}, current_status)
                    
                    # Save the analysis to the specified file path
                    output_file = "/home/dev2/projects/super-pilot/super-pilot/work/superpilot/superpilot/superpilot/examples/channels/vapi-calls/test_file.json"
                    save_call_analysis(analysis, output_file)
                    
                    # Print the analysis for verification
                    print("Final Call Analysis:")
...
def generate_mom(transcript, customer_name, agent_id, call_time):
    ...
    for message in transcript:
        ...
        if "fees" in text:
            key_discussions.append("Fee structure was discussed")
        if "schedule" in text:
