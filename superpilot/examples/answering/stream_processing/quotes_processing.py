...
from super_store.chat.models import UnpodAnswer
from super_store.chat.models import UnpodAnswerPiece
from super_store.chat.models import UnpodQuote
from super_store.chat.models import UnpodQuotes
from super_store.chat.models import LlmDoc
from super_store.configs.chat_configs import QUOTE_ALLOWED_ERROR_PERCENT
from super_store.prompts.constants import ANSWER_PAT
from super_store.prompts.constants import QUOTE_PAT
from super_store.prompts.constants import UNCERTAINTY_PAT
from super_store.search.models import InferenceChunk
from super_store.utils.logger import setup_logger
from super_store.utils.text_processing import clean_model_quote
from super_store.utils.text_processing import clean_up_code_blocks
from super_store.utils.text_processing import extract_embedded_json
...
def match_quotes_to_docs(
    ...
    return UnpodQuotes(quotes=unpod_quotes)
...
def process_answer(
    ...
    if answer == UNCERTAINTY_PAT or not answer:
        ...
        return UnpodAnswer(answer=None), UnpodQuotes(quotes=[])

    logger.info(f"Answer: {answer}")
    if not quote_strings:
        logger.debug("No quotes extracted from raw output")
        return UnpodAnswer(answer=answer), UnpodQuotes(quotes=[])
    ...
    return UnpodAnswer(answer=answer), quotes
...
def process_model_tokens(
    ...
    is_json_prompt: bool = True,
) -> Generator[UnpodAnswerPiece | UnpodQuotes, None, None]:
            ...
            if is_json_prompt and _stream_json_answer_end(model_previous, token):
                ...
                if token:
                    try:
                        ...
                        yield UnpodAnswerPiece(
                            answer_piece=hold_quote + token[:answer_token_section]
                        )
                    except ValueError:
                        logger.error("Quotation mark not found in token")
                        yield UnpodAnswerPiece(answer_piece=hold_quote + token)
                yield UnpodAnswerPiece(answer_piece=None)
                continue
