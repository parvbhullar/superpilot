...
class ThumbnailGenarator:
    ...
    def check_host(self, image_path):
        ...
        inputs = processor(
            text=["a person present", "person is not present"],
            images=image,
            return_tensors="pt",
            padding=True,
        )
    ...
    def uncrop(self, image_path, left_pad, right_pad, top_pad, bottom_pad):
        ...
        data = {
            "extend_left": left_pad,
            "extend_right": right_pad,
            "extend_up": top_pad,
            "extend_down": bottom_pad,
        }
        tmp_image_name = "uncrop.jpg"
        response = requests.post(
            "https://clipdrop-api.co/uncrop/v1", headers=headers, files=files, data=data
        )
        if response.status_code == 200:
            ...
            raise Exception(
                "Error in uncropping the background, response code: {}".format(
                    response.status_code
                )
            )
        return tmp_image_name

    def add_white_pixels_to_image(
        self, image_path, left_pad, right_pad, top_pad, bottom_pad
    ):
    ...
    def genarate_image(self, prompt):
        ...
        response = requests.post(
            self.url_genarate_background, headers=headers, files=files
        )
        if response.status_code == 200:
            with open(tmp_image_name, "wb") as f:
                f.write(response.content)
            ...
            raise Exception(
                "Error in genarating the background, response code: {}".format(
                    response.status_code
                )
            )
        return tmp_image_name
    ...
    def remove_background(self, image_path):
        ...
        response = requests.post(
            self.remove_background_url, headers=headers, files=files
        )
        if response.status_code == 200:
            with open(tmp_image_name, "wb") as f:
                f.write(response.content)
            ...
            raise Exception(
                "Error in removing the background, response code: {}".format(
                    response.status_code
                )
            )
        return tmp_image_name
    ...
    def background_fill(self, image_path, backgoround_prompt):
        ...
        response = requests.post(
            self.url_replace_background, headers=headers, files=files, data=data
        )
        if response.status_code == 200:
            with open(tmp_image_name, "wb") as f:
                f.write(response.content)
            ...
            if pixels > 0:
                if dimension == "width":
                    ...
                    left_pad = 0
                    right_pad = 0
                tmp_image_name = self.uncrop(
                    tmp_image_name, left_pad, right_pad, top_pad, bottom_pad
                )
            ...
            raise Exception(
                "Error in replacing the background, response code: {}".format(
                    response.status_code
                )
            )
        return tmp_image_name
    ...
    def background_fill_photapi(self, image_path, backgoround_prompt):
        ...
        headers = {"x-api-key": self.phot_x_api_key, "Content-Type": "application/json"}
        data = {
    ...
    def genarate(self, output_image_name):
        try:
            ...
            if not host:
                ...
                tmp_image_name = self.background_fill(
                    tmp_image_name, "A plain white background"
                )
            pixels, dimension = self.calculate_pixels_to_add(tmp_image_name)
            if pixels > 50:
                if self.expand_dim:
                    if self.expand_dim in ["right", "left", "top", "bottom"]:
                        ...
                        raise ValueError(
                            "Invalid value for expand_dim, it should be one of ['right','left','top','bottom']"
                        )
                else:
                    if dimension == "width":
                        left_pad = pixels // 2
                ...
                tmp_image_name = self.add_white_pixels_to_image(
                    tmp_image_name, left_pad, right_pad, top_pad, bottom_pad
                )
                tmp_image_name = self.background_fill(
                    tmp_image_name, "plain full white background"
                )
                tmp_image_name = self.background_fill(
                    tmp_image_name, self.backgoround_prompt
                )
            if host:
                tmp_image_name = self.background_fill(
                    tmp_image_name, self.backgoround_prompt
            ...
            return "Your thumbnail is Successfully Genarated and Saved as {}".format(
                tmp_image_name
            )
        except Exception as e:
            return "{}".format(e)
...
if __name__ == "__main__":
    ...
    for i in range(n_samples):
        ...
        thumbnail_genartor = ThumbnailGenarator(
            image_path,
            backgoround_prompt,
            text_prompt,
            font,
