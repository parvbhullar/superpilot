...
from super_store.prompts.chat_tools import (
    UNPOD_TOOL_DESCRIPTION,
)
from super_store.prompts.chat_tools import (
    UNPOD_TOOL_NAME,
)
...
from super_store.prompts.chat_tools import USER_INPUT
...
class UnpodChatModelOut(BaseModel):
    model_raw: str
    action: str
    action_input: str
...
def call_tool(
    model_actions: UnpodChatModelOut,
) -> str:
...
def form_tool_section_text(
    ...
    if retrieval_enabled and tools:
        tools.append(
            {"name": UNPOD_TOOL_NAME, "description": UNPOD_TOOL_DESCRIPTION}
        )
