...
from superpilot.framework.config import get_config, Config
from superpilot.framework.llm.base import ChatSequence
from superpilot.framework.llm.providers.openai import OPEN_AI_MODELS
from superpilot.framework.llm.utils import count_string_tokens, create_chat_completion
from superpilot.framework.helpers.logs import logger
from superpilot.framework.helpers.utils import batch
...
def summarize_text_web_driver(
    ...
    summary, chunks = summarize_text(text, CFG, question=question)
    print(summary)
    return summary

    summaries = []
    chunks = list(split_text(text, for_model=CFG.fast_llm_model, config=CFG))
