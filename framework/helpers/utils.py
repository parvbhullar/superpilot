...
def clean_input(config: Config, prompt: str = "", talk=False):
    try:
        ...
        # https://github.com/Significant-Gravitas/Superpilot/pull/4799/files/3966cdfd694c2a80c0333823c3bc3da090f85ed3#r1264278776
        answer = session.prompt(ANSI(prompt), handle_sigint=False)
        return answer
    except KeyboardInterrupt:
        logger.info("You interrupted Superpilot")
...
def get_bulletin_from_web():
    try:
        response = requests.get(
            "https://raw.githubusercontent.com/Significant-Gravitas/Superpilot/master/BULLETIN.md"
        )
...
def get_latest_bulletin() -> tuple[str, bool]:
    ...
    news_header = Fore.YELLOW + "Welcome to Superpilot!\n"
    if new_bulletin or current_bulletin:
        news_header += (
            "Below you'll find the latest Superpilot News and updates regarding features!\n"
            "If you don't wish to see this message, you "
            "can run Superpilot with the *--skip-news* flag.\n"
        )
