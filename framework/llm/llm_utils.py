...
def create_chat_completion(
    ...
    if response is None:
        logger.typewriter_log(
            ...
            Fore.RED,
            "Superpilot has failed to get a response from OpenAI's services. "
            + f"Try running Superpilot again, and if the problem the persists try running it with `{Fore.CYAN}--debug{Fore.RESET}`.",
        )
        logger.double_check()
        if cfg.debug_mode:
